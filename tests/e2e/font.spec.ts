...
test("한국어 글꼴을 외부 요청 없이 자체 제공", async ({ page }) => {
  ...
  await page.goto("/");
  await expect(page.getByRole("heading", { name: "연결 설정" })).toHaveCSS(
    "font-family",
    /Noto Sans KR/,
  );
  ...
  await expect(page.locator(".revision").first()).toHaveCSS("font-family", /Noto Sans KR/);
  await page.getByRole("button", { name: "Phase 2", exact: true }).click();
  await page.getByRole("button", { name: "파일 src/process.ts", exact: true }).click();
  for (const selector of [".code-pane h4", ".code-pane pre", ".rawdiff pre"]) {
    await expect(page.locator(selector).first()).toHaveCSS("font-family", /Noto Sans KR/);
  }
