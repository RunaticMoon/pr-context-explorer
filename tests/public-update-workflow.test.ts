...
test('public update Mac jobs receive explicit real update acceptance inputs', () => {
 ...
 for(const job of [w.split('  build:\n')[1].split('  macos26-install:')[0],w.split('  macos26-install:\n')[1].split('  publish:')[0]]) {
  ...
  assert.match(job,/export PRCE_PUBLIC_UPDATE_MANIFEST="\$GITHUB_WORKSPACE\/release\/public-mac.json"/);
 }
