...
export function EngineSetupPanel({
  ...
  return (
    <section className="engine-setup" aria-label="로컬 AI 엔진 설정">
      <header>
        <h3>로컬 AI 엔진</h3>
        <button
          type="button"
        ...
        <article key={engine.providerId}>
          <label>
            <input
              ...
              disabled={!engine.installed || busy}
          ...
          <dl>
            <dt>설치</dt>
            <dd>{engine.installed ? "감지됨" : "지원하는 설치를 찾지 못함"}</dd>
            <dt>버전</dt>
            ...
            <dt>격리 실행</dt>
            <dd>
              {engine.isolation.available && engine.isolation.runtimeVerified
                ? "확인됨"
            ...
            engine.localAuth !== "advanced" && (
              ...
              >
                <p>
                  ...
                  발급한 공식 토큰을 입력하세요. 자동 로그인이나 Keychain 읽기는
                  하지 않습니다.
                </p>
                <a
                ...
                <button type="submit" disabled={busy || !engine.candidateId}>
                  이번 세션에 토큰 사용
                </button>
                <p>
